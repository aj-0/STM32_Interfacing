...
/* Private user code ---------------------------------------------------------*/
/* USER CODE BEGIN 0 */
void Tx(uint8_t *m,uint16_t len)
{
    HAL_UART_Transmit(&huart2,m,len,HAL_MAX_DELAY);
}
int __io_putchar(int ch) { ITM_SendChar(ch); return ch; }

void handshake_cmd(void )
{
  uint8_t Handshake[] =
  {
  0xEF,0x01, //header
...
printf("RET=%d\r\n",ret);
     if  (ret==HAL_OK)
  {
	...
	printf("HANDSHAKE not working \n");
}


void fp_capture(void )
{
  ...
  printf("put Finger\r\n");
HAL_Delay(2500);
  Tx(getImage_cmd, sizeof(getImage_cmd));
  printf("rmve Finger\r\n");
...
void imgtocharbuff1(void)
{
  uint8_t genchar[] =
  {
      ...
      0x01, // pid
      0x00,0x04, //len
      0x02,0x01, // cmd & buffer id
      0x00,0x08 //cheksum
  };
  uint8_t rx[20];

  Tx(genchar, sizeof(genchar));
  ret = HAL_UART_Receive(&huart2, rx, 12, 1000);
  if(ret==HAL_OK)
    {
	  ...
	  printf("character file failed\n");

}

void imgtocharbuff2(void)
 {uint8_t genchar2[] =
  {
      0xEF,0x01, //header
      0xFF,0xFF,0xFF,0xFF, //addr
      0x01, // pid
      0x00,0x04, //len
      0x02,0x02, // cmd & buffer id
      0x00,0x09 //cheksum
  };
 uint8_t rx[20];

  Tx(genchar2, sizeof(genchar2));
  ret = HAL_UART_Receive(&huart2, rx, 12, 1000);
  if(ret==HAL_OK)
    {
...
//	   };
//
//	   Tx(getImage3_cmd, sizeof(getImage3_cmd));
//
//	   ret = HAL_UART_Receive(&huart2, rx, 12, 1000);
//
//	   if(ret==HAL_OK)
//	   {
//	       printf("img capture RX : ");
//
//	       for(int i=0;i<12;i++)
//	           printf("%02X ",rx[i]);
//
